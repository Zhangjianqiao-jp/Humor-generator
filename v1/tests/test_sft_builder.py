...
def test_build_sft_from_fake_csv(tmp_path: Path) -> None:
    ...
    pd.DataFrame([
        ...
        {"image_id": "1", "caption": "", "score": "oops"},
        {"image_id": "2", "caption": "a funny dog", "score": 0.8},
    ]).to_csv(caption_csv, index=False)

    build_sft_dataset(image_csv, caption_csv, out_dir, "image_id", "image_path", None, "image_id", "caption", "score",
                      threshold=0.0)
    combined = read_jsonl(out_dir / "sft_train.jsonl") + read_jsonl(out_dir / "sft_val.jsonl") + read_jsonl(
        out_dir / "sft_test.jsonl")
