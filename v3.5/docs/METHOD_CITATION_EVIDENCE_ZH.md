...
| A5 per-channel K/V/O + receiver-native target-span alignment | `configs/pilot/cross_attention_semantic_phase_a5.yaml`、`cross_attention.py` 的 `projection_mode=per_channel`、`cross_attention_bridge.py` 的 `semantic_target_alignment` | E0（代码与 real-trace smoke）；E3（完整训练/outer 尚未完成）/E2（组合变体） | 表示对齐后再做冻结接收器下游训练的原则见 [BLIP-2](https://proceedings.mlr.press/v202/li23q)；memory/current-state 对齐见 [HistAlign](https://aclanthology.org/2023.emnlp-main.179/)；多源 attention 背景见 [Libovický & Helcl 2017](https://aclanthology.org/P17-2031/)；gated receiver cross-attention 背景见 [Flamingo](https://proceedings.neurips.cc/paper_files/paper/2022/hash/960a172bc7fbf0177ccccbb411a7d800-Abstract-Conference.html) | “三类 humor channel + 独立 K/V/O + target-span cosine + 当前 counterfactual loss”没有被单篇论文验证，属于本项目 repair candidate。论文只能写“受上述原则启发”；只有完整训练、sealed outer gate 和 caption 盲评完成后，才可写本项目实测结果。 |
