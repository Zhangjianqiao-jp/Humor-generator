...
def inject_position_ids(
    position_ids: torch.Tensor,
    *,
    slot_count: int,
    insertion_index: int,
) -> torch.Tensor:
    """Insert sequential text-like MRoPE positions and shift the suffix."""
    if position_ids.ndim != 3 or position_ids.shape[0] != 3:
        raise ValueError("Qwen position_ids must be [3,B,T]")
    if slot_count < 1:
        raise ValueError("slot_count must be positive")
    if not 0 < insertion_index < position_ids.shape[-1]:
        raise ValueError("insertion index must lie strictly inside the sequence")
    start = position_ids[:, :, insertion_index : insertion_index + 1]
    offsets = torch.arange(slot_count, device=position_ids.device).view(1, 1, -1)
    slots = start + offsets
    return torch.cat(
        [
            position_ids[:, :, :insertion_index],
            slots,
            position_ids[:, :, insertion_index:] + slot_count,
        ],
        dim=-1,
    )
...
def assert_smoke_budget(
    *,
    ...
    if formal_training_enabled:
        raise RuntimeError("v3.5 is engineering-smoke-only until the scientific gates pass")
